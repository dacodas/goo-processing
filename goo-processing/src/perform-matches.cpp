...
int try_match(const char* target, const char* regex_expression)
{
    PCRE2_SPTR subject = reinterpret_cast<PCRE2_SPTR>(target);
    PCRE2_SPTR pattern = reinterpret_cast<PCRE2_SPTR>(regex_expression);
...
int main(int argc, char* argv[])
{
    ...
    {
        printf("Entry %d : ", ++line_counter);
        int rc;
        for (int i = 0; i < sizeof(potential_matches)/sizeof(*potential_matches); ++i)
        {
            rc = try_match(line.c_str(), potential_matches[i]);
            if ( rc == 0 )
                break;
        }

        if ( rc != 0 )
            printf("No match for %s", line.c_str());
